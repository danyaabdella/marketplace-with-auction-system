...
export async function POST(req) {
  ...
  try {
    ...
    const { amount, adData, user } = body;
    if (!amount || !adData || !adData.adId || !user) {
    ...
    const requestBody = {
      ...
      customization: {
        title: "Ad Payment",
        description: `Payment for advertisement: ${adData.product.productName}`.slice(0, 50).replace(/[^\w\s.-]/g, ""),
      },
    };
    ...
    if (!chapaResponse.ok || data.status !== "success") {
      ...
      await Advertisement.findByIdAndUpdate(adData.adId, { paymentStatus: "FAILED" });
      return new Response(
        JSON.stringify({ message: "Chapa API error", details: data.message || "Unknown error" }),
        { status: 400 }
      );
    }

    await Advertisement.findByIdAndUpdate(adData.adId, { paymentStatus: "PAID" });
    ...
    if (body?.adData?.adId) {
      await Advertisement.findByIdAndUpdate(body.adData.adId, { paymentStatus: "FAILED" });
    }
