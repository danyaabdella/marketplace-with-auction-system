...
export async function PUT(req) {
  try {
    ...
    if (newBidAmount <= bid.bids[userBidIndex].bidAmount) {
      return new Response(
        JSON.stringify({
          message: "New bid amount must be higher than current bid",
        }),
        { status: 400 }
      );
    }
    ...
    return new Response(
      JSON.stringify({ message: "Bid updated successfully", bid }),
      { status: 200 }
    );
  } catch (error) {
    return new Response(
      JSON.stringify({ message: "Failed to update bid", error: error.message }),
      { status: 500 }
    );
  }
}
...
export async function DELETE(req) {
  try {
    ...
    if (!auctionId) {
      return new Response(
        JSON.stringify({ message: "Auction ID is required" }),
        { status: 400 }
      );
    }
    ...
    return new Response(
      JSON.stringify({ message: "Bid deleted successfully", bid }),
      { status: 200 }
    );
  } catch (error) {
    return new Response(
      JSON.stringify({ message: "Failed to delete bid", error: error.message }),
      { status: 500 }
    );
  }
}
